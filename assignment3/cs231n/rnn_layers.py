...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    curr_h = x @ Wx + b
    next_h = np.tanh(prev_h @ Wh + curr_h)
...
def rnn_step_backward(dnext_h, cache): # dnext_h is the upstream gradient
    ...
    dWx = ((dnext_h * (1 - next_h ** 2)).T @ x).T
    dWh = ((dnext_h * (1 - next_h ** 2)).T @ prev_h).T
...
def rnn_backward(dh, cache):
    ...
    for t in range(T - 1, -1, -1):
      ...
      dx[:, t, :] = dx_t
      # Must accmulate the gradients from shared parameters makes sense for remembering
      dWx += dWx_t
      dWh += dWh_t
      db += db_t
...
def word_embedding_backward(dout, cache):
    ...
    dW = np.zeros((V, D))
    np.add.at(dW, x, dout) # The result of dW[x]: N by T by D is an array where each element in x is replaced by the 
