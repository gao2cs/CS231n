...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pe = pe.reshape(max_len, embed_dim)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # Torch Batch matrix multiplication (Multiplying the inside by 1.0 to avoid taking the square root of an integer)
        attn_scores = torch.matmul(Q, K.transpose(2, 3)) / torch.sqrt(torch.tensor(E / H, dtype=torch.float32)) 
        if attn_mask is not None:
            attn_scores = attn_scores.masked_fill(attn_mask == 0, float("-inf"))
        
        # Convert scores to probability summing up to 1.0 along each row, along T
        attn_scores = F.softmax(attn_scores, dim=-1) 
        Y = torch.matmul(self.attn_drop(attn_scores), V) # (N, H, S, T) to (N, H，S, E // H)
        Y = Y.transpose(1, 2).reshape(N, S, E) # (N, S, E) :Only consecutive terms can be multiplied
        output = self.proj(Y) # (N, S, E) to (N, S, E)
