...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        caption_embeddings = self.embedding(captions) # (N, T, E)
        caption_embeddings = self.positional_encoding(caption_embeddings)
        
        image_features = self.visual_projection(features).unsqueeze(1) # (N, 1, E)  # Maintain same dimension of 3
        tgt_mask = torch.tril(torch.ones(T, T, device=caption_embeddings.device, dtype=caption_embeddings.dtype))
        features =  self.transformer(tgt=caption_embeddings, memory=image_features, tgt_mask=tgt_mask)
