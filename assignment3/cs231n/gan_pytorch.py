...
def sample_noise(batch_size, dim, seed=None):
    ...
    return -2.0 * torch.rand((batch_size, dim)) + 1.0
...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    loss_real = bce_loss(logits_real, torch.ones_like(logits_real).view(-1))
    loss_fake = bce_loss(logits_fake, torch.zeros_like(logits_fake).view(-1))
...
def generator_loss(logits_fake): 
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    loss = bce_loss(logits_fake, torch.ones_like(logits_fake).view(-1))
    pass
